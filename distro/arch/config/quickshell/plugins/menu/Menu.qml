...
Item {
  ...
  readonly property var providers: ({
    "fonts": {
      ...
      icon: "",
      volatile: true,
      actionFor: function(value) { return "fuzi-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); fuzi-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "fuzi-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fuzi-menu"
      ...
      Column {
        ...
        Rectangle {
          ...
          Text {
            ...
            text: root.filterText || (root.dmenuActive ? (root.dmenuPrompt + "...") : Style.searchPlaceholder)
