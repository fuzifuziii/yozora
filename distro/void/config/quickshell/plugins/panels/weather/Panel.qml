...
Panel {
  id: root
  moduleName: "fuzi.weather"
  ipcTarget: "fuzi.weather"
  manageIpc: false
  ...
  // fuzi-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/fuzi/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["fuzi-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["fuzi-weather-location", "--set", name]
    else
      locationSaveProc.command = ["fuzi-weather-location", "--clear"]
  ...
  // Mirrors fuzi-weather-icon's wttr.in code → nerd-font glyph mapping.
