# Tailscale Fuzi Widget

Native Fuzi bar widget for Tailscale.
...
`fuzi-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `fuzi tailscale receive`
...
This widget ships as first-party plugin `fuzi.tailscale`. Add it with `fuzi plugin enable fuzi.tailscale`, then place it with `fuzi bar move fuzi.tailscale` if desired.
