...
Panel {
  id: root
  moduleName: "fuzi.monitor"
  ipcTarget: "fuzi.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (fuzi-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["fuzi-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "fuzi-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["fuzi-display-text-size", String(px)]
  ...
  Process {
    ...
    // `fuzi-brightness-display` races the hardware/driver and can
