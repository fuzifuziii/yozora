...
Item {
  ...
  readonly property string stayAwakeStateDir: home + "/.local/state/fuzi/indicators"
  ...
  readonly property string screensaverClass: "org.fuzi.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("fuzi idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(fuzi-shell lock isLocked 2>/dev/null) == \"true\" ]] || fuzi-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "fuzi-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "fuzi-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/fuzi/indicators\" && touch \"$HOME/.local/state/fuzi/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/fuzi/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/fuzi/indicators\"; if [[ -f $HOME/.local/state/fuzi/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
