...
Item {
  ...
  function logEvent(event) {
    ...
    console.log("fuzi lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "fuzi-lock-preview"
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/fuzi-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "fuzi-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "fuzi-brightness-keyboard off; fuzi-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/fuzi-lock-password"
    watchChanges: true
    printErrors: false
