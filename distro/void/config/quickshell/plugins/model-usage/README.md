...
`fuzi plugin disable fuzi.model-usage`.
...
- IPC: `fuzi-shell fuzi.model-usage <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/quickshell/shell.json`. The
top-level keys can be set with
`fuzi bar set fuzi.model-usage <key> <value>`:
...
```bash
fuzi bar set fuzi.model-usage refreshIntervalSec 300 --json
fuzi bar set fuzi.model-usage syncDir '~/Sync/model-usage'
```
...
```bash
fuzi bar set fuzi.model-usage providers '{
  "claude": {
    "enabled": true,
