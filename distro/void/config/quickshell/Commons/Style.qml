...
QtObject {
  ...
  // component follows the fontconfig alias `fuzi-font-set` writes.
  ...
  // alias path keeps working when the user runs `fuzi font set`), but
  ...
  // FUZI_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("FUZI_MENU_FONT")
  ...
  readonly property QtObject bar: QtObject {
    ...
    readonly property int sizeVertical:   root.barToken("size-vertical",   40)
    readonly property int iconSlot:       root.barToken("icon-slot",       26)
    readonly property int iconCanvas:     root.barToken("icon-canvas",     18)
    readonly property int iconFont:       root.barToken("icon-font",       14)
    readonly property int statusSlot:     root.barToken("status-slot",     22)
  }
  ...
  // Resolve the fontconfig alias to a concrete family name. `fuzi font
  ...
  // `fuzi toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/fuzi/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
