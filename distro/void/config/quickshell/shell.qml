...
ShellRoot {
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        left: [{ id: "fuzi.workspaces" }],
        center: [{ id: "fuzi.clock", format: "dddd HH:mm" }],
        right: [
          { id: "fuzi.tray" },
          { id: "fuzi.keyboard-layout" },
          { id: "fuzi.bluetooth" },
          { id: "fuzi.network" },
          { id: "fuzi.audio" },
          { id: "fuzi.power" }
  ...
  Component.onCompleted: {
    console.log("fuzi-shell paths",
      "fuziPath=" + shell.fuziPath,
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("fuziPath" in target) target.fuziPath = shell.fuziPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("fuziPath" in inst) inst.fuziPath = shell.fuziPath
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("fuziPath" in item) item.fuziPath = shell.fuziPath
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("fuzi.image-picker", payload) ? "ok" : "unknown"
    }
